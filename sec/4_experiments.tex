...
\paragraph{Frequency-Driven Detection Bias Analysis.}
We analyze the relationship between input frequency characteristics and detector outputs to understand whether high-frequency regions systematically induce false or overconfident detections.

Let $x$ denote a detection region. We compute its high-frequency energy using the Laplacian operator:
\begin{equation}
    e_f(x) = \frac{1}{|\Omega|} \sum_{p \in \Omega} |\Delta x(p)|^2,
\end{equation}
where $\Delta x(p)$ denotes the discrete Laplacian response.

To normalize across images, we use a dataset-level scaling factor:
\begin{equation}
    \hat{f}(x) = \min\!\left(\frac{e_f(x)}{Q_{75}(e_f)}, 2\right),
\end{equation}
where $Q_{75}$ denotes the 75th percentile of frequency energy.

\paragraph{Frequency--Objectness Relationship.}
We study whether high-frequency regions induce inflated detection confidence. Empirically, we observe a positive association between frequency energy and detector responses, where texture-rich regions tend to produce higher objectness scores. This suggests that detectors exhibit sensitivity to high-frequency patterns even in the absence of semantic objects.

\paragraph{Frequency--Detection Behavior Analysis.}
We further analyze how frequency energy influences detection density and confidence. Specifically, we observe that regions with higher frequency energy tend to produce more detections and higher confidence values in baseline models. This indicates that high-frequency textures act as a spurious cue for objectness estimation.

\paragraph{Calibration Effect.}
To mitigate this behavior, we introduce a post-hoc calibration function:
\begin{equation}
    s' = s \cdot \exp(-\alpha \hat{f}) \cdot \exp(-\beta o) \cdot \frac{1}{\gamma(a)},
\end{equation}
where $o$ is a proxy objectness score and $a$ is bounding box area.

This calibration reduces confidence in high-frequency and small-region detections, suppressing hallucinated predictions without retraining.

\paragraph{Empirical Findings.}
Our implementation shows:
\begin{itemize}
    \item A reduction in false positives from 444 to 324 (27.03\% decrease),
    \item A decrease in hallucination rate from 0.209 to 0.164,
    \item A left-shift in confidence distributions after calibration,
    \item Reduced detection density in high-frequency regions.
\end{itemize}

These results confirm that frequency-aware post-hoc calibration effectively mitigates texture-induced false detections in real-time object detectors.
